...
#endif
#endif
	LCD_sendCommand(DISPLAY_ON_CURSOR_ON);	/*turn on the LCD and display cursor*/
}
/*
...
#if (LCD_MODE == 8)
	_delay_us(1); 					/*delay tas (50 ~ ns)*/
	SET_BIT(CONTROL_PORT , E); 		/*enable pin = 1*/
	_delay_us(1); 					/*delay tpw - tdsw (290~_ - 100~_ ns = 190~_ns)*/
	DATA_PORT = a_command;			/*write command*/
	_delay_us(1); 					/*delay tdsw	(100~_ns)*/
